...
# 阶段1：阶段性放量吸筹（新逻辑）
# 对比"最近N天日均量"vs"之前M天日均量"，识别阶段性放量
VOLUME_BASELINE_PERIOD = 20       # 均量基准计算周期（放量期之前的日均量）
VOLUME_COMPARE_PERIOD = 3         # 当前均量计算周期（最近几天的日均量）
VOLUME_PHASE_RATIO = 1.5          # 阶段性放量比率阈值（最近均量/之前均量 >= 1.5）
VOLUME_PHASE_MIN_DAYS = 3         # 阶段性放量最少天数要求
